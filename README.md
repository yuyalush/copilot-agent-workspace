...
Below are actual slides generated from a single prompt: *"Azure App Service の概要をMSLearnで調査してPPTXで出力してください。"*

![Slide 1](samples/slide01.svg)
![Slide 2](samples/slide02.svg)
![Slide 3](samples/slide03.svg)
![Slide 4](samples/slide04.svg)
![Slide 5](samples/slide05.svg)
![Slide 6](samples/slide06.svg)
![Slide 7](samples/slide07.svg)
![Slide 8](samples/slide08.svg)
![Slide 9](samples/slide09.svg)
![Slide 10](samples/slide10.svg)
...
- **Two Fine-grained PATs** — see [Secrets / PAT Setup](#secrets--pat-setup) below
...
This project requires **two** Fine-grained Personal Access Tokens (PATs) stored as GitHub Actions secrets.  
Two PATs are necessary because `Copilot Requests` permission is only available on **Public repositories** scoped PATs, while repository-specific permissions (Issues, Contents, etc.) require a **specific-repository** scoped PAT.

| Secret Name | Repository Access | Permissions | Used By |
|-------------|-------------------|-------------|---------|
| `COPILOT_FG_TOKEN` | **Public Repositories** (all) | `Copilot Requests: read` | SDK execution (`copilot-poc.yml`, healthcheck Step 5) |
| `REPO_FG_TOKEN` | **Specific repository** only | `Metadata: read`, `Actions: read/write`, `Contents: read/write`, `Issues: read/write`, `Pull requests: read/write` | Issue creation + Copilot Coding Agent assignment (healthcheck Steps 8–9) |

### How to create the PATs

1. Go to **GitHub → Settings → Developer settings → Personal access tokens → Fine-grained tokens**
2. Create **`COPILOT_FG_TOKEN`**:
   - Repository access: **Public Repositories**
   - Permissions → Account permissions → **Copilot Requests: Read**
3. Create **`REPO_FG_TOKEN`**:
...
- PAT uses minimal permissions — split into two tokens with least-privilege scoping (see [Secrets / PAT Setup](#secrets--pat-setup))
...
│   ├── workflows/
│   │   ├── copilot-poc.yml          # Main SDK runner pipeline
│   │   └── copilot-healthcheck.yml  # Daily E2E healthcheck (SDK → SVG → PPTX) + auto Issue + Copilot fix
...
│   ├── esm-loader.mjs              # Custom ESM resolve hook (vscode-jsonrpc compat)
...
├── package.json                     # ESM module config + @github/copilot-sdk dependency
...
以下は「Azure App Service の概要をMSLearnで調査してPPTXで出力してください。」というプロンプト 1 つで生成された実際のスライドです。
...
- **Fine-grained PAT × 2 本** — 下記 [Secrets / PAT セットアップ](#secrets--pat-セットアップ) 参照
...
このプロジェクトでは **2 本** の Fine-grained PAT を GitHub Actions secrets に登録する必要があります。  
`Copilot Requests` パーミッションは **Public repositories** スコープの PAT でしか付与できないため、リポジトリ固有の操作（Issue 作成等）には別の PAT が必要です。

| Secret 名 | リポジトリアクセス | パーミッション | 用途 |
|-----------|-------------------|---------------|------|
| `COPILOT_FG_TOKEN` | **Public Repositories**（全公開リポジトリ） | `Copilot Requests: read` | SDK 実行（`copilot-poc.yml`、healthcheck Step 5） |
| `REPO_FG_TOKEN` | **特定リポジトリ** のみ | `Metadata: read`, `Actions: read/write`, `Contents: read/write`, `Issues: read/write`, `Pull requests: read/write` | Issue 作成 + Copilot Coding Agent アサイン（healthcheck Steps 8–9） |

#### PAT の作成手順

1. **GitHub → Settings → Developer settings → Personal access tokens → Fine-grained tokens** を開く
2. **`COPILOT_FG_TOKEN`** を作成:
   - Repository access: **Public Repositories**
   - Permissions → Account permissions → **Copilot Requests: Read**
3. **`REPO_FG_TOKEN`** を作成:
...
- PAT は最小権限で 2 本に分離— 用途ごとに最小スコープを適用（[PAT セットアップ](#secrets--pat-セットアップ) 参照）
...
│   ├── workflows/
│   │   ├── copilot-poc.yml          # メイン SDK 実行パイプライン
│   │   └── copilot-healthcheck.yml  # E2E ヘルスチェック（SDK → SVG → PPTX）+ 自動 Issue + Copilot 修正
│   ├── skills/                      # 4 つのドメイン知識スキル
│   └── instructions/                # ファイルパターン別の品質基準
├── src/
│   ├── copilot-sdk-runner.mjs       # SDK ベースの Copilot ランナー（リトライ付き）
│   ├── esm-loader.mjs              # カスタム ESM resolve フック（vscode-jsonrpc 互換）
│   ├── register-loader.mjs         # ESM ローダー登録
│   ├── svg2pptx.py                  # SVG → ネイティブ PPTX 変換（1,093 行）
│   └── requirements.txt
├── webapp/                          # Blazor Server ファイル配布 UI（.NET 10）
├── presentations/                   # プロジェクト紹介スライド
├── output/                          # 生成物（.gitignore）
├── package.json                     # ESM モジュール設定 + @github/copilot-sdk 依存
├── AGENTS.md                        # エージェント方針＆ワークスペース設計
