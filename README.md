...
- Two Fine-grained PATs (see details below)
  - `COPILOT_FG_TOKEN`: Public repositories + `Copilot Requests` Read-only — for Copilot SDK execution
  - `GitHub__Token`: All repositories + `Actions` Read and write — for Web App workflow trigger
...
- PAT uses minimal permissions — two tokens with separate roles:
  - `COPILOT_FG_TOKEN`: `Copilot Requests` Read-only (Public repositories)
  - `GitHub__Token`: `Actions` Read and write (All repositories)
...
- 2種類の Fine-grained PAT（詳細は下記）
  - `COPILOT_FG_TOKEN`: Public repositories + `Copilot Requests` Read-only — Copilot SDK 実行用
  - `GitHub__Token`: All repositories + `Actions` Read and write — Web アプリからのワークフロートリガー用
